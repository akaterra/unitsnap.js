...
describe('Spy', () => {
  describe('when spies on function', () => {
    it('should use arguments annotations in ES5 declaration', () => {
      ...
      expect(unitsnap.state(spied).args).toEqual({'*': [4, 5], a: 1, b: 2, c: 3});
    });
    ...
    it('should use arguments annotations in ES6 declaration', () => {
      ...
      expect(unitsnap.state(spied).args).toEqual({a: 1, b: 2, c: 3, dd: 4, e: undefined, ff: undefined, g: [5]});
    });
    ...
    it('should use arguments annotations in ES6 declaration with no brackets single argument', () => {
      ...
      expect(unitsnap.state(spied).args).toEqual({'*': [2, 3, 4, 5], a: 1});
    });
    ...
    it('should spy on resolved promise', (done) => {
      ...
      spied().then((result) => {
        ...
        expect(unitsnap.state(spied).isAsync).toBe(true);
        expect(unitsnap.state(spied).isException).toBe(false);
    ...
    it('should spy on rejected promise', (done) => {
      ...
      spied().catch((result) => {
        expect(result).toBe(1);
        expect(unitsnap.state(spied).isAsync).toBe(true);
        expect(unitsnap.state(spied).isException).toBe(true);
  ...
  describe('when spies on method', () => {
    it('should use arguments annotations in ES5 class declaration', () => {
      ...
      expect(unitsnap.state(spied.a).args).toEqual({'*': [], a: 1});
    });
  ...
  describe('when spies on descriptor', () => {
    ...
    it('should arguments annotations in ES5 class getter declaration', () => {
      ...
      expect(unitsnap.state(Object.getOwnPropertyDescriptor(spied, 'a').get).args).toEqual({'*': []});
    });
    ...
    it('should arguments annotations in ES5 class setter declaration', () => {
      ...
      expect(unitsnap.state(Object.getOwnPropertyDescriptor(spied, 'a').set).args).toEqual({'*': [], value: 5});
    });
